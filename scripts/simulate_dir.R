...
for(sim in sims){
  ...
  if(distr_str == "dir"){
    # Ktot <- m
    tmp_prob <- rdiric(n = 1, shape = rep(theta/Ktot,Ktot))
    tmp_x <- rmultinom(n, size = 1, prob = tmp_prob)
    tmp_pre <- apply(tmp_x, MARGIN = 2, function(x) which(x == 1))
    x_pre <- tmp_pre
  } 
  if(distr_str == "DP"){
    tmp_pre <- PYP(theta, 0, n)
    x_pre <- tmp_pre
  }
  ...
  results[(index-1)*Nsim_post + 1:Nsim_post, 3] <- theta
  results[(index-1)*Nsim_post + 1:Nsim_post, 4:9] <- rep(c(parameters_ord, parameters_std, parameters_ordDP),each = Nsim_post)
  results[(index-1)*Nsim_post + 1:Nsim_post, 10:13] <- rep(c(parameters_lsK, parameters_lsX1),each = Nsim_post)
  ...
  for(i_method in 1:5){
    ...
    if(i_method == 3){
      ...
      # EKpost_f <- parameters[1] * log(n+m) # the formula should condition on K, this is wrong.
    } else {
      EK_f <- EK_PY(n, parameters[1], parameters[2])
      # EKpost_f <- EK_PY(n+m, parameters[1], parameters[2]) # the formula should condition on K, this is wrong.
    }
  ...
  for(sim_post in 1:Nsim_post){
    cat(sim_post, " ")
    if(distr_str == "dir"){
      tmp_x <- rmultinom(m, size = 1, prob = tmp_prob)
      tmp_post <- apply(tmp_x, MARGIN = 2, function(x) which(x == 1))
    } 
    if(distr_str == "DP"){
      tmp_post <- PYP_post(tmp_pre, theta, 0, m)
    }
    ...
    results[(index-1)*Nsim_post + sim_post,"Kpost"] <- length(clusters$unique_clusters)
