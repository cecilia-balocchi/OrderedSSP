...
KcurveCIs_est <- array(0, dim = c(length(methods_str), length(Kns_post_CI),2))
W1curveCIs_est <- array(0, dim = c(length(methods_str), length(W1ns_CI),2))
...
library(ggplot2)
library(tidyr)
tmp_Ks_bibtex = tidyr::pivot_longer(as.data.frame(Ks_perr_bibtex), cols = colnames(Ks_perr_bibtex), names_to = "method")
...
p = ggplot(results) + geom_boxplot(aes(x = method, y = value)) +
  ggh4x::facet_nested( ~ variable, scale = "free") + scale_y_log10() + ylab("Percentage error")+
