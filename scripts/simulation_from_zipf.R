...
for(which_order in c("arrival_weight", "astable")){
  for(batch in 1:n_batches){
    rstudioapi::jobRunScript("scripts/simulate_zipf_FB.R", name = paste0("simulate_zipf_batch",batch), importEnv = TRUE)
  }
}
...
save(list = c("results3_tidy","results_comb"), file = "results/results_zipf_FB.Rdata")
