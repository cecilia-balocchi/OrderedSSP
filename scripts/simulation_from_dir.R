...
percentage_error <- TRUE
results_tidy <- data.frame(order = NA, sim = NA, true_theta = NA, 
  ...
  for(batch in 1:n_batches){
    ...
    for(sim in sims){
      for(i_method in 1:5){
        ...
        tmp = list(order_str,sim,results_comb$true_theta[index_sim_post],"W1curve",substr(method_str,2,10)[i_method],RMSE_W1curve_tmp/Nsim_post)
        names(tmp) <- colnames(results_tidy)
        results_tidy <- rbind(results_tidy, tmp)
        
        tmp = list(order_str,sim,results_comb$true_theta[index_sim_post],"W1",substr(method_str,2,10)[i_method],RMSE_W1_tmp/Nsim_post)
        names(tmp) <- colnames(results_tidy)
        results_tidy <- rbind(results_tidy, tmp)
        
        tmp = list(order_str,sim,results_comb$true_theta[index_sim_post],"A1",substr(method_str,2,10)[i_method],RMSE_A1_tmp/Nsim_post)
        names(tmp) <- colnames(results_tidy)
        results_tidy <- rbind(results_tidy, tmp)
        
        tmp = list(order_str,sim,results_comb$true_theta[index_sim_post],"K",substr(method_str,2,10)[i_method],RMSE_K_tmp/Nsim_post)
        names(tmp) <- colnames(results_tidy)
        results_tidy <- rbind(results_tidy, tmp)
        
        tmp = list(order_str,sim,results_comb$true_theta[index_sim_post],"W1_A1",substr(method_str,2,10)[i_method],mean(RMSE_W1_A1_tmp, na.rm = T))
        names(tmp) <- colnames(results_tidy)
        results_tidy <- rbind(results_tidy, tmp)
        
        tmp = list(order_str,sim,results_comb$true_theta[index_sim_post],"W1_B1",substr(method_str,2,10)[i_method],mean(RMSE_W1_B1_tmp, na.rm = T))
        names(tmp) <- colnames(results_tidy)
        results_tidy <- rbind(results_tidy, tmp)
        
        ### 
        if(percentage_error){
          tmp1 = list(order_str,sim,results_comb$true_theta[index_sim_post],"W1curve2",substr(method_str,2,10)[i_method],
                      sqrt(mean(( (W1_curve_tmp/Nsim_post - W1curve_fs[[paste0(sim,method_str[i_method])]])/(W1_curve_tmp/Nsim_post) )^2)) )
          tmp2 = list(order_str,sim,results_comb$true_theta[index_sim_post],"W12",substr(method_str,2,10)[i_method],abs(W1_tmp/Nsim_post-EW1_f)/(W1_tmp/Nsim_post))
          tmp3 = list(order_str,sim,results_comb$true_theta[index_sim_post],"K2",substr(method_str,2,10)[i_method],abs(K_tmp/Nsim_post-EK_f)/(K_tmp/Nsim_post))
          tmp4 = list(order_str,sim,results_comb$true_theta[index_sim_post],"W1_A12",substr(method_str,2,10)[i_method],abs(mean(W1_A1_tmp, na.rm = TRUE) - res_f[4])/mean(W1_A1_tmp, na.rm = TRUE))
          tmp5 = list(order_str,sim,results_comb$true_theta[index_sim_post],"W1_B12",substr(method_str,2,10)[i_method],abs(mean(W1_B1_tmp, na.rm = T) - res_f[5])/mean(W1_B1_tmp, na.rm = TRUE))
          tmp6 = list(order_str,sim,results_comb$true_theta[index_sim_post],"A12",substr(method_str,2,10)[i_method],abs(A1_tmp/Nsim_post-pA1_f)/(A1_tmp/Nsim_post))
          
        } else {
          tmp1 = list(order_str,sim,results_comb$true_theta[index_sim_post],"W1curve2",substr(method_str,2,10)[i_method],
                      sqrt(mean(( W1_curve_tmp/Nsim_post - W1curve_fs[[paste0(sim,method_str[i_method])]] )^2)))
          tmp2 = list(order_str,sim,results_comb$true_theta[index_sim_post],"W12",substr(method_str,2,10)[i_method],abs(W1_tmp/Nsim_post-EW1_f))
          tmp3 = list(order_str,sim,results_comb$true_theta[index_sim_post],"K2",substr(method_str,2,10)[i_method],abs(K_tmp/Nsim_post-EK_f))
          tmp4 = list(order_str,sim,results_comb$true_theta[index_sim_post],"W1_A12",substr(method_str,2,10)[i_method],abs(mean(W1_A1_tmp, na.rm = TRUE) - res_f[4]))
          tmp5 = list(order_str,sim,results_comb$true_theta[index_sim_post],"W1_B12",substr(method_str,2,10)[i_method],abs(mean(W1_B1_tmp, na.rm = T) - res_f[5]))
          tmp6 = list(order_str,sim,results_comb$true_theta[index_sim_post],"A12",substr(method_str,2,10)[i_method],abs(A1_tmp/Nsim_post-pA1_f))
        }
...
save(list = c("results3_tidy","results_tidy","results_comb"), file = "results/results_DP.Rdata")
