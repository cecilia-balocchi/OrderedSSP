...
for(sim in sims){
  ...
  results[(index-1)*Nsim_post + 1:Nsim_post, 3] <- par
  ...
  for(sim_post in 1:Nsim_post){
    cat(sim_post, " ")
    if(distr_str == "zipf"){
      tmp_post <- rzeta(n = m, shape = par)
    } 
    if(distr_str == "finzipf"){
      tmp_post <- rzipf(n = m, N = Ktot, shape = par) 
    }
    ...
    if(which_order == "int"){
      clusters$rank <- match(clusters$unique_clusters, sort(clusters$unique_clusters, decreasing = T))
    }
    if(which_order == "arrival_weight"){
      clusters$weights <- NA
